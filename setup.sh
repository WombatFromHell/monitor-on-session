...
if [ "$1" == "install" ]; then
  ...
  chmod 0755 "$HOME"/.local/bin/{monitor-dbus-session-state,fix-gsync,openrgb-load,on-session}.sh
  ...
  rm -f "$HOME"/.local/bin/{monitor-dbus-session-state,fix-gsync,openrgb-load,on-session}.sh
  systemctl --user daemon-reload
